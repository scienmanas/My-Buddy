...
export default function Chat(props) {
  ...
  const handleShare = () => { setShareWindow(true); };

  const handleCloseShare = () => { setShareWindow(false); };

  const toggleSidebar = () => { setIsOpen(!isOpen); };

  const handleNewChat = () => {};

  const hanldeChangeChat = (id) => { setcurrentChat(id); };
  ...
  return (
    ...
    handleCloseShare} />
      ...
      <div className="in-contents flex flex-row w-full relative">
        <div
  className =
      {`sidebar absolute sm:relative sm:min-h-screen duration-200 transition-all ${
          isOpen ? "w-72 sm:w-fit" : "hidden"}`} > <
      SideBar
  chatList = {chatList} currentChat = {currentChat} isOpen =
      {isOpen} hanldeChangeChat =
  { hanldeChangeChat } />
        </div >
      <div className =
           "closebar hidden text-white sm:flex min-h-screen items-center">
      <CloseBar isOpen = {isOpen} toggleSidebar =
       { toggleSidebar } />
        </div>
      <div className =
           "content-side w-full custom-height sm:min-h-screen flex flex-col justify-between -ml-4">
      <div className = "user-options-bar flex-none">< ContentSide
  handleShare = {handleShare} chatList = {chatList} currentChat =
  { currentChat } />
          </div >
      <div className = "conversation-area overflow-auto flex-grow p-4">
      <Conversation geminiAns = {geminiAns} chatHistory =
       { chatHistory } />
          </div><div className = "chat-area">
      <ChatInput handlePrompt = {handlePrompt} prompt =
       { prompt } />
          </div></div>
      </div>< /div>
  );
